...
import { asMomentUtc, asMomentUtcGuessEnd, dateTimeUTCForGooglenYahoo, dateTimeUTCForOutlook, IInviteDB } from './nivite-sdk';

export class NiviteSdk {
  public static google(invite: IInviteDB): string {
    const details = {
      action: 'TEMPLATE',
      dates: asMomentUtc(invite.timeFrom, dateTimeUTCForGooglenYahoo)
        + '/' + asMomentUtc(invite.timeTo ? invite.timeTo :
          asMomentUtcGuessEnd(invite.timeFrom), dateTimeUTCForGooglenYahoo),
  ...
  public static outlook(invite: IInviteDB): string {
    const details = {
      body: invite.longMsg + '\n\n~' + invite.hostName + '\nnivite.com',
      enddt: asMomentUtc(invite.timeTo ? invite.timeTo
        : asMomentUtcGuessEnd(invite.timeFrom), dateTimeUTCForOutlook),
      location: invite.addrText,
  ...
  public static yahoo(invite: IInviteDB): string {
    const details = {
      desc: invite.longMsg + '\n\n~' + invite.hostName + '\nnivite.com',
      et: asMomentUtc(invite.timeTo ? invite.timeTo
        : asMomentUtcGuessEnd(invite.timeFrom), dateTimeUTCForGooglenYahoo),
